...
fn apply_floyd_steinberg_dither(
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let mut distances: Vec<(u32, usize, [u8; 3])> = palette.iter().enumerate()
                .map(|(idx, &color)| {
...
fn distribute_error(
    ...
    distribute(error_buffer, x, y, width, height, quant_error, 1, 0, 7.0 / 16.0);
    distribute(error_buffer, x, y, width, height, quant_error, -1, 1, 3.0 / 16.0);
    distribute(error_buffer, x, y, width, height, quant_error, 0, 1, 5.0 / 16.0);
    distribute(error_buffer, x, y, width, height, quant_error, 1, 1, 1.0 / 16.0);
}
...
pub fn process_png_sequence(glob_pattern: &str) -> Result<Vec<ProcessedFrame>, BytePusherError> {
    ...
    let mut file_paths: Vec<_> = glob(glob_pattern)?
